...
import br.com.Produtos.exceptions.NonexistentEntityException;
...
public class ProdutosJpaController implements Serializable {
    ...
    public void edit(Produtos produtos) throws NonexistentEntityException, Exception {
        ...
        try {
            ...
            if (msg == null || msg.length() == 0) {
                Integer idProduto = produtos.getIdProduto();
                if (findProdutos(idProduto) == null) {
                    throw new NonexistentEntityException("The produtos with id " + idProduto + " no longer exists.");
                }
            }
            throw ex;
        } finally {
    ...
    public void destroy(Integer idProduto) throws NonexistentEntityException {
        EntityManager em = null;
        try {
            em = getEntityManager();
            em.getTransaction().begin();
            Produtos produtos;
            try {
                produtos = em.getReference(Produtos.class, idProduto);
                produtos.getIdProduto();
            } catch (EntityNotFoundException enfe) {
                throw new NonexistentEntityException("The produtos with id " + idProduto + " no longer exists.", enfe);
            }
            em.remove(produtos);
            em.getTransaction().commit();
        } finally {
